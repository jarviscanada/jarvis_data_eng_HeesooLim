...
public class HashJMap<K, V> implements JMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        //init this.table
        if (this.table == null) {
            this.table = (Node<K, V>[]) new Object[capacity()]; /* Not type-safe. */
        }
...
//                return;
        ...
        if (this.size > threshold) {
            Node<K, V>[] biggerTable = (Node<K, V>[]) new Object[table.length * 2];
            System.arraycopy(table, 0, biggerTable, 0, table.length);
        }
        return null;
    }
